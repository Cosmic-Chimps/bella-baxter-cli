...
) : AsyncCommand<GenerateSecretsCodeSettings>
{
    ...
    )
    {
        ...
        {
            "csharp" => GenerateDotnet(manifest, className, namespaceName),
            "fsharp" => GenerateFsharp(manifest, className, namespaceName),
    ...
    private static string GenerateDotnet(SecretsManifest manifest, string className, string? ns)
    {
        ...
        sb.AppendLine("// Generated by: bella secrets generate csharp");
        ...
        sb.AppendLine("// DO NOT edit manually — re-run bella secrets generate csharp to update.");
        sb.AppendLine();
    ...
    private static string GenerateFsharp(SecretsManifest manifest, string className, string? ns)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("// Generated by: bella secrets generate fsharp");
        sb.AppendLine($"// Project:      {manifest.ProjectSlug}/{manifest.EnvironmentSlug}");
        sb.AppendLine($"// Generated at: {manifest.GeneratedAt:yyyy-MM-dd HH:mm:ss} UTC");
        sb.AppendLine("// DO NOT edit manually — re-run bella secrets generate fsharp to update.");
        sb.AppendLine();
        if (!string.IsNullOrWhiteSpace(ns))
        {
            sb.AppendLine($"namespace {ns}");
        ...
        sb.AppendLine();
        sb.AppendLine("    let private getRequired (key: string) =");
        sb.AppendLine("        match System.Environment.GetEnvironmentVariable(key) with");
        sb.AppendLine("        | null | \"\" -> failwith $\"Required secret '{key}' is not set.\"");
        ...
        foreach (var s in manifest.Secrets)
        {
            if (s.Description is not null)
                sb.AppendLine($"    /// <summary>{EscapeXmlComment(s.Description)}</summary>");
            var fsExpr = GetFsharpBinding(s.Key, s.Type);
            sb.AppendLine($"    let {ToPascalCase(s.Key)} = {fsExpr}");
        }
        return sb.ToString();
    }

    private static string GetFsharpBinding(string key, string type) =>
        type.ToLowerInvariant() switch
        {
            "int" => $"getRequired \"{key}\" |> int",
            "bool" => $"getRequired \"{key}\" |> System.Boolean.Parse",
            "float" => $"getRequired \"{key}\" |> float",
            "double" => $"getRequired \"{key}\" |> float",
            "uri" => $"getRequired \"{key}\" |> System.Uri",
            "guid" => $"getRequired \"{key}\" |> System.Guid.Parse",
            "base64" => $"getRequired \"{key}\" |> System.Convert.FromBase64String",
            _ => $"getRequired \"{key}\"",
        };
