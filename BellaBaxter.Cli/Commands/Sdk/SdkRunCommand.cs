...
namespace BellaCli.Commands.Sdk;

/// <summary>
/// bella sdk run — inject Bella connection credentials as env vars and spawn a subprocess.
///
/// The child process contains the Bella SDK, which owns its secret lifecycle:
/// polling, caching, hot-reload via onUpdate callbacks, and E2EE decryption.
/// The CLI only hands off the minimal credentials the SDK needs to talk to Baxter.
///
/// This is fundamentally different from 'bella run', which fetches all secrets
/// itself and injects them as plain env vars. Use:
///   bella run     → for scripts, legacy apps, and containers (CLI owns the lifecycle)
///   bella sdk run → for SDK-powered long-running services (app owns the lifecycle)
///
...
) : AsyncCommand<SdkRunCommand.Settings>
{
    ...
    )
    {
            ...
            if (string.IsNullOrEmpty(projectSlug) || string.IsNullOrEmpty(environmentSlug))
            {
                output.WriteError(
                    "Project and environment are required for JWT auth.\n"
                        ...
                        + "  1. bella sdk run -p my-project -e dev -- <command>\n"
        ...
        psi.Environment.Remove("BELLA_BAXTER_PRIVATE_KEY");
    ...
    /// </summary>
    protected static (string? Project, string? Environment)? ReadBellaFile(string startDir)
    {
        var dir = startDir;
        while (true)
        {
